...
class DecisionMatrixDeterministic:
    ...
    def decide(
        ...
        valid_candidates = self._filter_hard_constraints(
            action_candidates, constraints or []
        )
        ...
        penalties = self._compute_soft_penalties(valid_candidates, constraints or [])
    ...
    def _filter_hard_constraints(
        self, candidates: List[Dict], constraints: List[Constraint]
    ) -> List[Dict]:
    ...
    def _build_decision_matrix(
        self, candidates: List[Dict], criteria: DecisionCriteria
    ) -> np.ndarray:
        """
    ...
    def _get_violations(
        self, candidate: Dict, constraints: List[Constraint]
    ) -> List[str]:
    ...
    def adapt_criteria(self, performance_feedback: float, learning_rate: float = 0.1):
        """
        ...
        if performance_feedback < 0.5:
            ...
            total = (
                self.criteria.Q_value
                + self.criteria.efficiency
                + self.criteria.safety
                + self.criteria.modularity
            )
...
def create_safety_constraint(max_magnitude: float = 2.0) -> Constraint:
    ...
    return Constraint(name="max_magnitude", type=ConstraintType.HARD, check_fn=check)
...
def create_risk_constraint(max_risk: float = 0.8) -> Constraint:
    ...
    return Constraint(
        name="max_risk", type=ConstraintType.SOFT, check_fn=check, penalty_weight=2.0
    )
...
if __name__ == "__main__":
    ...
    dmd = DecisionMatrixDeterministic(
        criteria=DecisionCriteria(
            Q_value=1.0, efficiency=0.4, safety=0.6, modularity=0.2
        )
    )
    ...
    candidates = [
        {
            ...
            "Q_value": 0.6,
            "miem": {"efficiency": 0.7, "risk": 0.2, "modularity": 0.8},
        },
        {
            "id": "aggressive",
            "action": np.random.randn(16) * 2.5,
            "Q_value": 0.9,
            "miem": {"efficiency": 0.9, "risk": 0.85, "modularity": 0.4},
        },
        {
            "id": "balanced",
            "action": np.random.randn(16) * 0.8,
            "Q_value": 0.75,
            "miem": {"efficiency": 0.75, "risk": 0.4, "modularity": 0.7},
        },
    ]
