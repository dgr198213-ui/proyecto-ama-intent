...
"""
    ...
    def step_1_verify_requirements(self) -> bool:
        ...
        if sys.version_info < (3, 7):
            self.log_msg(
                f"Python 3.7+ requerido (actual: {sys.version_info.major}.{sys.version_info.minor})",
                "ERROR",
            )
            return False
        self.log_msg(
            f"Python {sys.version_info.major}.{sys.version_info.minor} ✓", "SUCCESS"
        )
    ...
    def step_2_copy_installers(self) -> bool:
        ...
        for script in scripts:
            ...
            if src.exists():
                ...
                self.log_msg(
                    f"Script no encontrado (se creará stub): {script}", "WARNING"
                )
    ...
    def step_3_run_master_installer(self) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self.log_msg(
                    f"Master Installer falló (code: {result.returncode})", "ERROR"
                )
                if result.stderr:
                    print(result.stderr)
                return False
...
Ejemplos:
  ...
  python ama_one_click_deploy.py --skip-tests
    ...
    parser.add_argument(
        "--path", default=".", help="Ruta de destino (default: directorio actual)"
    )
    parser.add_argument(
        "--skip-tests", action="store_true", help="Omitir ejecución de tests"
    )

    args = parser.parse_args()

    deployer = OneClickDeployer(target_path=args.path, skip_tests=args.skip_tests)
