...
class ThalamicFilter:
    ...
    def __init__(
        ...
        self.state = KalmanState(
            ...
            H=(
                np.eye(dim_obs, dim_state)
                if dim_obs <= dim_state
                else np.random.randn(dim_obs, dim_state)
            ),
    ...
    def filter(self, y: np.ndarray) -> Tuple[np.ndarray, dict]:
        ...
        metrics = {
            ...
            "innovation_norm": (
                np.linalg.norm(y - (self.state.H @ x_pred)) if self.initialized else 0.0
            ),
        }
...
def create_thalamic_filter(
    embedding_dim: int = 384, latent_dim: Optional[int] = None
) -> ThalamicFilter:
    """
...
if __name__ == "__main__":
    ...
    # Crear filtro
    thalamus = ThalamicFilter(
        dim_state=1, dim_obs=1, process_noise=0.01, measurement_noise=0.3**2
    )
    ...
    print(
        f"RMSE con filtro: {np.sqrt(np.mean((np.array(filtered) - true_signal)**2)):.4f}"
    )
