...
class AMAPhaseIntegrator:
    ...
    def fase2_process(self, fase1_output, user_input):
        ...
        if cognitive_out["action"] == "search":
            search_results = self.bus.call("BDC-Search", {"op": "search", "query": user_input, "k": 3})
    ...
    def fase3_process(self, fase2_output, alternatives=None):
        ...
        if alternatives:
            ...
            dmd_out = self.bus.call("DMD", {"alternatives": alternatives, "weights": weights})
        
        # Evaluación LFPI (simulada para el ejemplo)
        lfpi_out = self.bus.call("LFPI", {
            "gain": 0.8,
            "loss": 0.1,
            "feedback": 0.7,
            "amplitude": 1.0
        ...
        return {
            "dmd": dmd_out,
            "lfpi": lfpi_out,
            "best_response": dmd_out["best"] if dmd_out else "Respuesta generada por defecto"
        }
