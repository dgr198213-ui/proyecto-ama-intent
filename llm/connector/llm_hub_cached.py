...
class MultiHeadLatentAttention(nn.Module):
    ...
    def forward(
        ...
        K_full = K_full.view(
            batch_size, seq_len_k_full, self.num_heads, self.d_k
        ).transpose(1, 2)
        V_full = V_full.view(
            batch_size, seq_len_k_full, self.num_heads, self.d_k
        ).transpose(1, 2)
        ...
        memory_saved = (
            seq_len_k * self.d_model - seq_len_k * self.latent_dim
        ) * batch_size
...
class CorticalAttentionMLAIntegration:
    ...
    def __init__(self, dim: int = 512, beta: float = 0.5, latent_dim: int = 128):
        ...
        self.mla = MultiHeadLatentAttention(
            d_model=dim, num_heads=8, latent_dim=latent_dim
        )

        # Habilitar cache por defecto
        self.mla.enable_kv_cache()

    def compute_attention_long_context(
        self,
        delta: np.ndarray,
        context: np.ndarray,
...
"""
        ...
        if hasattr(request, "context") and request.context:
            ...
            if (
                "KNOWLEDGE GRAPH" in request.context
                or "PROJECT RULES" in request.context
            ):
...
async def demo_caching_and_mla():
    ...
    config = {"cache_size": 50, "cache_ttl_hours": 24}
