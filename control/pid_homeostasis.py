...
class PIDController:
    ...
    def get_statistics(self) -> Dict:
        ...
        return {
            ...
            'settled': np.std([h['error'] for h in recent[-10:]]) < 0.1 if len(recent) >= 10 else False
        }
...
class HomeostaticSystem:
    ...
    def update_all(self,
                   ...
                   wm_load: float,
                   performance: float) -> Dict[ControlledParameter, float]:
        """
...
if __name__ == "__main__":
    ...
    for t in range(20):
        ...
        if t < 5 or t % 5 == 0:
            ...
            print(f"  Outputs:")
            print(f"    τ (exploration) = {params[ControlledParameter.EXPLORATION]:.3f}")
            print(f"    η (learning_rate) = {params[ControlledParameter.LEARNING_RATE]:.4f}")
            print(f"    λ (attention) = {params[ControlledParameter.ATTENTION_LAMBDA]:.3f}")
            print(f"    ρ (risk_aversion) = {params[ControlledParameter.RISK_AVERSION]:.3f}")
            print()
