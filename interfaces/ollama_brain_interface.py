...
class GovernedLocalLLM:
    ...
    def chat(self, user_message: str) -> Dict:
        ...
        print(
            f"  {'✓' if intake_approved else '✗'} Intake: {brain_result['execution_mode']}"
        )
        ...
        llm_response, llm_metadata = self.llm.generate(
            prompt=user_message, system_prompt=shadow_prompt
        )
        ...
        output_approved = (
            validation_result["execution_mode"] != "safe_mode" and intent_preserved
        )

        print(
            f"  {'✓' if output_approved else '✗'} Validación: {validation_result['execution_mode']}"
        )
        ...
        print(f"\n{'='*70}")
        print(
            f"{'✅ RESPUESTA APROBADA' if output_approved else '⚠ RESPUESTA BLOQUEADA'}"
        )
        ...
        return {
            "response": (
                llm_response
                if output_approved
                else "Respuesta bloqueada por gobernanza."
            ),
            "approved": output_approved,
            "intent_preserved": intent_preserved,
            "brain_state": validation_result,
    ...
    def _build_shadow_prompt(
        self, user_message: str, intent: Intent, brain_state: Dict
    ) -> str:
        """
...
Responde de forma clara, precisa y útil."""
    ...
    def get_statistics(self) -> Dict:
        ...
        return {
            ...
            "approval_rate": (
                np.mean([c["approved"] for c in self.conversation_history])
