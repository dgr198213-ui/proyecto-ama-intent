...
class SubgraphRetriever:
    ...
    def retrieve_for_query(
        self, query_type: QueryType, entity: str, max_depth: int = 2
    ) -> SubgraphContext:
        """
    ...
    def _retrieve_entity_info(self, entity: str) -> SubgraphContext:
        ...
        for rel in self.kg.relations:
            if rel.source == entity:
                ...
                target_e = next(
                    (e for e in self.kg.entities if e.name == rel.target), None
                )
                if target_e:
                    related.append(
                        {
    ...
    def _retrieve_usage(self, entity: str) -> SubgraphContext:
        ...
        for rel in self.kg.relations:
            if rel.target == entity and rel.relation_type.value == "calls":
                callers.append(
                    {"source": rel.source, "target": rel.target, "type": "calls"}
                )
                caller_e = next(
                    (e for e in self.kg.entities if e.name == rel.source), None
                )
                if caller_e:
        ...
        for dep_type, dep_list in deps.items():
            for dep in dep_list:
                context.relations.append(
                    {"source": entity, "target": dep, "type": dep_type}
                )
        return context
        ...
        for affected_name in list(affected)[:20]:
            ...
            if aff_e:
                context.related_entities.append(
                    {
                        "name": aff_e.name,
                        "type": aff_e.type.value,
                        "file": aff_e.file_path,
                    }
                )
        return context
    ...
    def _retrieve_patterns(self, entity: str) -> SubgraphContext:
        ...
        patterns = [
            ...
            and (
                e.metadata.get("class") == entity
                or e.metadata.get("function") == entity
            )
        ]
        for p in patterns:
            context.related_entities.append(
                {
                    "name": p.name,
                    "type": "pattern",
                    "pattern_type": p.metadata.get("pattern_type"),
                }
            )
        return context
