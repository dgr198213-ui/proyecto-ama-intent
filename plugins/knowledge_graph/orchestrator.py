...
class KnowledgeGraphOrchestrator:
    ...
    async def build_knowledge_graph(self, force: bool = False):
        ...
        self.kg = ProjectKnowledgeGraph(
            project_path=self.config.project_path, base_uri="http://ama-intent.dev/kg"
        )
        ...
        print(
            f"✅ KG construido: {metrics['total_functions']} funciones, {metrics['total_classes']} clases"
        )
    ...
    async def analyze_code_with_kg_context(
        ...
        patterns = [
            ...
            if e.type == NodeType.PATTERN and e.file_path == file_path
        ]

        return {
            "file_path": file_path,
            "entities": [
                {"name": e.name, "type": e.type.value, "line": e.line_number}
                for e in file_entities
            ],
            "dependencies": dependencies,
            "impact_analysis": impact,
            "patterns": [
                {"name": p.name, "type": p.metadata.get("pattern_type")}
                for p in patterns
            ],
