...
class CodeEntity:
    ...
    def to_uri(self, base_uri: str) -> str:
        ...
        return f"{base_uri}/{self.type.value}/{safe_path}_{self.name}_{self.line_number}"
...
class PatternDetector:
    ...
    def _detect_singleton(self):
        ...
        for entity in self.entities:
            ...
            methods = [e for e in self.entities 
    ...
    def _detect_factory(self):
        ...
        for entity in self.entities:
            ...
            if any(entity.name.startswith(prefix) 
    ...
    def _detect_decorator_pattern(self):
        ...
        for entity in self.entities:
            ...
            init_method = next((e for e in self.entities 
                               if e.type == NodeType.METHOD 
                               and e.metadata.get('class') == entity.name 
                               and e.name == '__init__'), None)
    ...
    def _detect_observer(self):
        ...
        for entity in self.entities:
            ...
            methods = [e.name for e in self.entities 
                      if e.type == NodeType.METHOD 
                      and e.metadata.get('class') == entity.name]
...
class ProjectKnowledgeGraph:
        ...
        for pattern in file_patterns:
            for file_path in self.project_path.glob(pattern):
                ...
                try:
                    ...
                    analyzer = ASTAnalyzer(str(file_path.relative_to(self.project_path)), source)
    ...
    def impact_analysis(self, entity_name: str, max_depth: int = 3) -> Set[str]:
        ...
        try:
            ...
            nodes = nx.single_source_shortest_path_length(reverse_graph, entity_name, cutoff=max_depth)
    ...
    def get_complexity_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_classes": len([e for e in self.entities if e.type == NodeType.CLASS]),
            "total_functions": len([e for e in self.entities if e.type in [NodeType.FUNCTION, NodeType.METHOD]]),
            "total_patterns": len([e for e in self.entities if e.type == NodeType.PATTERN]),
