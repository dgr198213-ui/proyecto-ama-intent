...
class CognitiveMemory:
    ...
    def store(self, state, result) -> int:
        ...
        cursor.execute('''
            ...
            (timestamp, intent, context, result, confidence, created_at, relevance_score)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ''', (
    ...
    def retrieve(self, query: str, limit: int = 10, min_relevance: float = 0.0) -> List[Dict[str, Any]]:
        """
    ...
    def _check_consolidation(self, intent: str):
        ...
        if count >= self.consolidation_threshold:
            ...
            cursor.execute('''
                ...
                (pattern_key, pattern_data, frequency, last_accessed, consolidated_at, importance)
                VALUES (?, ?, ?, ?, ?, ?)
            ''', (
                intent,
                json.dumps({
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        cursor.execute('SELECT COUNT(*), AVG(confidence), AVG(relevance_score) FROM short_term_memory')
