...
class EngineBus:
    ...
    def call(self, name: str, payload: Optional[Dict[str, Any]] = None) -> EngineResult:
        ...
        if name not in self._engines:
            res = EngineResult(
                ok=False, engine=name, error=f"Engine '{name}' not registered"
            )
        ...
        self._metrics["avg_time"] = (self._metrics["avg_time"] * (n - 1) + elapsed) / n
    ...
    def pipeline(self, names: List[str], payload: Dict[str, Any]) -> EngineResult:
        ...
        return last if last else EngineResult(ok=True, engine="pipeline", data=cur)
