...
def example_muonclip_training():
    ...
    from training.optimizers import MuonClipConfig, MuonClipOptimizer
    ...
    for step in range(10):
        ...
        if step % 5 == 0:
            print(
                f"Step {stats.step}: Loss={stats.loss:.4f}, "
...
async def example_data_synthesizer():
    ...
    trajectories = await synthesizer.generate_trajectories(
        num_trajectories=10, variants_per_bug=3
    )
...
def example_context_caching():
    ...
    mla = MultiHeadLatentAttention(
        d_model=512, num_heads=8, latent_dim=128  # Compresión 4x
    )
