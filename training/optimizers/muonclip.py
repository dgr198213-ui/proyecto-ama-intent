...
class AttentionMonitor:
    ...
    def compute_max_logit(self, attention_scores: torch.Tensor, 
                         layer_idx: int) -> float:
        """
...
class MuonMomentum:
    ...
    def update(self, param_name: str, gradient: torch.Tensor, 
...
class MuonClipOptimizer:
    ...
    def __init__(self, model: nn.Module, config: MuonClipConfig):
        ...
        self.muon = MuonMomentum(
            ...
            ortho_enabled=config.momentum_orthogonalization
        )
    ...
    def step_with_monitoring(self, loss: torch.Tensor, 
                            attention_scores: Optional[Dict[int, torch.Tensor]] = None) -> TrainingStats:
        """
        ...
        if self.config.qk_clip_enabled and self.step % self.config.clip_frequency == 0:
            if self.attention_monitor.is_saturated():
                ...
                if qk_clipped:
                    print(f"✂️  QK-Clip aplicado en step {self.step} (max_logit={max_logit:.2f})")
    ...
    def get_training_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_saturation": np.mean([s.attention_saturation for s in self.stats_history])
        }
...
class RewardModelTrainer:
    ...
    def train_epoch(self) -> Dict[str, float]:
        ...
        for batch_idx, batch in enumerate(self.train_dataloader):
            ...
            if batch_idx % 50 == 0:
                print(f"Batch {batch_idx}, Loss: {stats.loss:.4f}, "
    ...
    def train(self, num_epochs: int = 3):
        ...
        print(f"✅ ZERO LOSS SPIKES FATALES" if summary['loss_spikes_detected'] == 0 else "⚠️  Spikes detectados")
