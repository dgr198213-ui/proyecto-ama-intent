...
class CorticalState:
    ...
    def _initialize_dynamics(self) -> CorticalDynamics:
        ...
        return CorticalDynamics(
            W_input=np.random.uniform(
                -limit_input, limit_input, (cfg.dim_latent, cfg.dim_input)
            ),
            W_recurrent=np.random.uniform(
                -limit_rec, limit_rec, (cfg.dim_latent, cfg.dim_latent)
            ),
            W_memory=np.random.uniform(
                -limit_mem, limit_mem, (cfg.dim_latent, cfg.dim_working_memory)
            ),
    ...
    def update(
        self, y_hat: np.ndarray, alpha: np.ndarray, w: Optional[np.ndarray] = None
        ...
        metrics = {
            ...
            "input_contribution": np.linalg.norm(z_input)
            / (np.linalg.norm(z_pre) + 1e-9),
            "recurrent_contribution": np.linalg.norm(z_rec)
            / (np.linalg.norm(z_pre) + 1e-9),
        }
    ...
    def compute_surprise(
        self, y_hat: np.ndarray, y_pred: np.ndarray
    ...
    def set_state(self, z_new: np.ndarray):
        ...
        if len(z_new) != self.config.dim_latent:
            raise ValueError(
                f"Dimensión incorrecta: esperado {self.config.dim_latent}, recibido {len(z_new)}"
            )
        self.z = z_new.copy()
...
if __name__ == "__main__":
    ...
    cortex = create_cortical_state(
        dim_latent=32, dim_input=64, dim_wm=16, activation="tanh", leak=0.05
    )
