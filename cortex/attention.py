...
class CorticalAttention:
    ...
    def adapt_lambda(self, performance_metric: float, target: float = 0.7):
        ...
        self.lambda_param = np.clip(self.lambda_param, 
                                    self.lambda_min, 
                                    self.lambda_max)
...
def create_attention_mechanism(dim: int, 
                               temperature: float = 1.0) -> CorticalAttention:
    """
...
if __name__ == "__main__":
    ...
    print(f"Proporción atendida: {np.linalg.norm(x_attended)/np.linalg.norm(x_input):.3f}")
