...
class PluginInstance:
    ...
    def execute(self, action: str, *args, **kwargs) -> Any:
        ...
        raise AttributeError(f"Plugin '{self.manifest.name}' no tiene acción '{action}'")
            ...
            if plugin_path.is_dir():
                ...
                if manifest_file.exists():
                    try:
                        ...
                        print(f"❌ Error cargando manifiesto de {plugin_path.name}: {e}")
    ...
    def content_service(self, action: str, **kwargs):
        ...
        if action == "generate_blog":
            ...
            return writer.generate_draft(
                kwargs.get('topic', ''),
                style=kwargs.get('style', 'tutorial')
            )
...
async def get_plugin_types():
    ...
    return [{"value": pt.value, "label": pt.name.replace("_", " ").title()} 
...
async def create_plugin_template(
    ...
    try:
        ...
        plugin_dir = plugin_manager.create_plugin_template(name, plugin_type_enum, author)
