...
async def credentials_dashboard(request: Request, db: Session = Depends(get_db)):
...
async def add_credential_form(
    ...
    available_services = manager.get_available_services()
    service_config = next(
        (s for s in available_services if s["name"] == service_name), None
    )
...
@router.post("/save")
async def save_credential(request: Request, db: Session = Depends(get_db)):
...
async def delete_credential(
    request: Request, credential_id: int, db: Session = Depends(get_db)
):
    """Eliminar credencial"""
    user = get_current_user_from_session(request, db)
    if not user:
...
async def toggle_credential(
    ...
    return {
        "status": "success",
        "is_active": new_state,
        "message": f"Credencial {state_text}",
    }


@router.get("/api/services")
async def get_available_services(request: Request, db: Session = Depends(get_db)):
...
@router.get("/api/user")
async def get_user_credentials(request: Request, db: Session = Depends(get_db)):
