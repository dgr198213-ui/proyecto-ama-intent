...
class CredentialsManager:
    ...
    def __init__(self, db_session: Session, secret_key: str = None):
        self.db = db_session
        self.secret_key = secret_key or os.getenv("CREDENTIALS_SECRET_KEY", "default-key-change-me")
    ...
    def save_credential(self, user_id: int, service_name: str, api_key: str, 
                       api_base: str = "", api_version: str = "", 
                       organization: str = "", project_id: str = "", region: str = "") -> Dict:
        """Guardar credencial cifrada"""
        from src.personal_dashboard.database import ServiceCredential
        
        # Verificar si ya existe
        existing = self.db.query(ServiceCredential).filter_by(
            user_id=user_id, 
    ...
    def get_credential(self, user_id: int, service_name: str, decrypt_key: bool = False) -> Optional[Dict]:
        """Obtener credencial (opcionalmente descifrada)"""
        from src.personal_dashboard.database import ServiceCredential
        
        credential = self.db.query(ServiceCredential).filter_by(
            user_id=user_id, 
            service_name=service_name,
            is_active=True
        ).first()
    ...
    def get_all_credentials(self, user_id: int) -> List[Dict]:
        ...
        credentials = self.db.query(ServiceCredential).filter_by(
            user_id=user_id
        ).order_by(ServiceCredential.service_name).all()
        ...
        return [
            {
                ...
                "fields": [
                    ...
                    {"name": "api_base", "label": "API Base URL", "type": "text", "required": False},
