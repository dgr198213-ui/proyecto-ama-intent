...
try:
    from qodeia_engines import BaseEngine, EngineBus
...
class AMAPhaseIntegrator:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        print(
            f"[INIT] AMA-Intent v2.0 inicializado con {len(self.bus.list_engines())} motores"
        )
    ...
    def _register_engines(self):
        ...
        self.bus.register(
            CognitiveBrainEngine(wm_size=self.config["cognitive_wm_size"])
        )
        ...
        # Registrar Phase-Integrator (wrapper para process_full)
        class PhaseIntegratorWrapper(BaseEngine):
            ...
            def _run(self, payload):
                return self.parent.process_full(payload.get("text", ""))

        self.bus.register(PhaseIntegratorWrapper(self))
    ...
    def fase1_process(
        ...
        # 1. Gobernanza AMA-G
        result_g = self.bus.call(
            "AMA-G",
            {"text": user_input, "requires_text": True, "metadata": metadata or {}},
        )
    ...
    def fase2_process(
        self, fase1_output: Dict[str, Any], context: Optional[Dict] = None
    ) -> Dict[str, Any]:
        """
        ...
        result_cog = self.bus.call(
            "Cognitive-Brain",
            {"text": user_input, "intent": fase1_output.get("intent")},
        )
        ...
        if action in ["search", "analyze"] and user_input:
            result_bdc = self.bus.call(
                "BDC-Search",
                {"op": "search", "query": user_input, "k": self.config["bdc_top_k"]},
            )
        ...
        result_mem = self.bus.call(
            "Associative-Memory", {"op": "query", "query": user_input, "k": 3}
        )

        associative_memory = []
        if result_mem.ok:
            associative_memory = result_mem.data.get("associative", {}).get(
                "results", []
            )
    ...
    def fase3_process(
        self, fase2_output: Dict[str, Any], alternatives: Optional[List[Dict]] = None
    ) -> Dict[str, Any]:
        """
        ...
        if alternatives:
            ...
            if result_dmd.ok:
                ...
                print(
                    f"  ✓ Best choice: {best_choice.get('name')} (score: {best_choice.get('score'):.3f})"
                )
    ...
    def consolidate_nightly(self, force: bool = False) -> Dict[str, Any]:
        ...
        for i, interaction in enumerate(self.medium_term):
            ...
            importance = intent_importance.get(
                interaction.get("intent", "general"), 0.5
            )
    ...
    def ingest_knowledge(self, documents: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # Ingest en BDC
        result_bdc = self.bus.call("BDC-Search", {"op": "ingest", "docs": documents})
