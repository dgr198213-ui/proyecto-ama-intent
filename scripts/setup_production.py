...
def create_production_env():
    ...
    if db_choice == "s":
        ...
        env_config["DATABASE_URL"] = (
            f"postgresql://{env_config['DB_USER']}:{env_config['DB_PASSWORD']}@{env_config['DB_HOST']}:{env_config['DB_PORT']}/{env_config['DB_NAME']}"
        )
    else:
    ...
    if redis_choice == "s":
        ...
        env_config["REDIS_HOST"] = (
            input("   Redis Host (default: localhost): ") or "localhost"
        )
        env_config["REDIS_PORT"] = input("   Redis Port (default: 6379): ") or "6379"
        env_config["REDIS_PASSWORD"] = (
            getpass.getpass("   Redis Password (opcional, enter para omitir): ") or ""
        )
    ...
    cors_origins = (
        input("   Orígenes permitidos (separados por coma, * para todos): ") or "*"
    )
    env_config["CORS_ORIGINS"] = cors_origins

    # Rate limiting
    print("\n   Rate Limiting (límite de peticiones)")
    env_config["RATE_LIMIT_PER_MINUTE"] = (
        input("   Peticiones por minuto por IP (default: 100): ") or "100"
    )
    env_config["RATE_LIMIT_PER_HOUR"] = (
        input("   Peticiones por hora por usuario (default: 1000): ") or "1000"
    )
    ...
    log_file = (
        input("   Archivo de log (default: logs/ama_dashboard.log): ")
        or "logs/ama_dashboard.log"
    )
    ...
    core_url = (
        input("   URL del Core (ej: http://localhost:8001): ")
        or "http://localhost:8001"
    )
    env_config["AMA_CORE_URL"] = core_url

    core_api_key = (
        getpass.getpass("   API Key del Core (opcional, enter para omitir): ") or ""
    )
    if core_api_key:
