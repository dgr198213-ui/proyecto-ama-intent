...
class BackupManager:
    ...
    def create_backup(self, backup_type: str = "auto") -> Optional[Path]:
        ...
        try:
            ...
            if not DatabaseBackup.backup_online(self.config.DB_PATH, db_backup_path):
            ...
            metadata = {
                ...
                "checksums": {"database": calculate_checksum(db_backup_path)},
            }
            ...
            try:
                ...
                if age_days <= self.config.RETENTION_DAYS:
                    ...
                    if not any(
                        ...
                        backup_path.unlink()
                        logger.info(
                            f"  ✓ Deleted (weekly duplicate): {backup_path.name}"
                        )
                    ...
                    if not any(
                        ...
                        if datetime.strptime(b.stem.split("_")[-2], "%Y%m%d").month
                        == month
                    ):
                        ...
                        backup_path.unlink()
                        logger.info(
                            f"  ✓ Deleted (monthly duplicate): {backup_path.name}"
                        )
                else:
                    # Demasiado antiguo
                    backup_path.unlink()
    ...
    def list_backups(self) -> List[Dict]:
        ...
        for backup_path in sorted(
            self.config.BACKUP_DIR.glob("ama_backup_*.tar.gz"), reverse=True
        ):
    ...
    def restore_backup(self, backup_path: Path, verify: bool = True) -> bool:
        ...
        try:
            ...
            current_db_backup = (
                ...
                / f"ama_dashboard_pre_restore_{datetime.now().strftime('%Y%m%d_%H%M%S')}.db"
            )
